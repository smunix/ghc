...
rnWcBody ctxt nwc_rdrs hs_ty
  ...
  where
    ...
    rn_ty env (HsForAllTy { hst_fvf = fvf, hst_bndrs = tvs
                          , hst_body = hs_body })
      = bindLHsTyVarBndrs (rtke_ctxt env) YesWarnUnusedForalls
                          Nothing tvs $ \ tvs' ->
...
bindLHsTyVarBndrs doc wuf mb_assoc tv_bndrs thing_inside
  ...
  where
    ...
    warn_unused tv_bndr fvs = case wuf of
      YesWarnUnusedForalls -> warnUnusedForAll doc tv_bndr fvs
